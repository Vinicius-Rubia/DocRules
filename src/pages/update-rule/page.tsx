...
export function UpdateRule() {
  ...
  const [rules, setRules] = useLocalStorage<Rule[]>(KEYS.RULES, []);
  ...
  if (!rule) {
    return (
      <div className="flex flex-col items-center justify-center text-center text-muted-foreground p-8 border border-dashed rounded-lg mt-4">
        <Info className="size-8 mb-4" />
        <h2 className="text-lg font-semibold">Regra não encontrada</h2>
        <p className="text-sm mb-4">
          Verifique se realmente existe essa regra.
        </p>
        <Button asChild>
          <NavLink to="/rules">Ver todas as regras</NavLink>
        </Button>
      </div>
    );
  }
  ...
  function onSubmit(data: AllRulesType) {
    const payload: Rule = { ...data, id: rule!.id };
    setRules((prev) => prev.map((r) => (r.id === rule!.id ? payload : r)));
    toast.success("Sucesso!", {
      description: "Sua regra foi atualizada com sucesso.",
    });
    navigate("/rules");
  }
          ...
          <div className="space-y-6">
            ...
            {form.formState.errors.contexts?.root && (
              <p className="text-destructive text-sm">
                {form.formState.errors.contexts.root.message}
              </p>
            )}
          </div>

          {fields.length > 0 && (
            <Button type="button" variant="outline" onClick={handleAddBehavior}>
              <PlusCircle className="mr-2 size-4" />
              Adicionar Novo Contexto
            </Button>
          )}
